...
function Notes() {
  ...
  useEffect(() => {
    ...
    async function loadNotes() {
      ...
      setError(null)
      try {
        const result = await apiRequest<PaginatedResponse<Note>>('/public/notes', {
          token,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
        <h1 className="text-xl font-semibold text-gray-900">Public Notes</h1>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        {!isLoading && !error && notes.length === 0 && (
          <p className="text-center text-sm text-gray-500">No public notes yet.</p>
        )}
